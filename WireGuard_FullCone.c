...
int InstallWireGuard(){
    ...
    printf("正在编译Fullcone. . . . . .\n");
    system("curl -sSL https://raw.githubusercontent.com/HXHGTS/WireGuardServer_Debian10/main/fullcone_iptables.sh | bash");
    printf("正在生成服务器配置. . . . . .\n");
    ...
    fprintf(server_config, "Address = 10.103.100.1/32\n");//服务器ip地址，修改需要同时修改客户端配置
    fprintf(server_config, "PostUp = iptables -A FORWARD -s 10.103.100.0/24 -j ACCEPT;iptables -A FORWARD -d 10.103.100.0/24 -j ACCEPT;iptables -t nat -A POSTROUTING -o eth0 ! -p udp -j MASQUERADE;iptables -t nat -A POSTROUTING -o eth0 -p udp -j FULLCONENAT --to-ports 10000-65535 --random-fully;iptables -t nat -A PREROUTING -i eth0 -p udp -m multiport --dports 10000:65535 -j FULLCONENAT\n");//服务器防火墙配置
    fprintf(server_config, "PostDown = iptables -D FORWARD -s 10.103.100.0/24 -j ACCEPT;iptables -D FORWARD -d 10.103.100.0/24 -j ACCEPT;iptables -t nat -D POSTROUTING -o eth0 ! -p udp -j MASQUERADE;iptables -t nat -D POSTROUTING -o eth0 -p udp -j FULLCONENAT --to-ports 10000-65535 --random-fully;iptables -t nat -D PREROUTING -i eth0 -p udp -m multiport --dports 10000:65535 -j FULLCONENAT\n");//服务器防火墙配置
    fprintf(server_config, "ListenPort = %d\n",ListenPort);//服务器监听端口
    fprintf(server_config, "MTU = %d\n", MTU);//服务器MTU
    fclose(server_config);
...
int AddUser() {
    ...
    system(command);
    server_config = fopen("/etc/wireguard/wg0.conf", "a");
    fprintf(server_config, "AllowedIPs = 10.103.100.%d/32\n", num);//客户端ip地址分配，不要修改
    ...
    fprintf(client_config, "Address = 10.103.100.%d/32\n", num, num);
    fprintf(client_config, "DNS = 10.103.100.1\n");
    fprintf(client_config, "MTU = %d\n", MTU);//客户端MTU
